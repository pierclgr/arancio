Executes a given command in a non-persistent session on the host platform's native shell — <field>_shell_label</field> on this machine — with optional timeout and working directory, ensuring proper handling and capturing of output.

## PRE-EXECUTION STEPS
Before executing the command, please follow these steps:
1. Directory Verification:
   - If the command creates new directories or files, first run a directory listing to verify the parent directory
     exists and is the correct location (e.g. for command `mkdir dir/folder`, first list `dir` to check that `dir`
     is the intended parent directory).
...
## USAGE
  ...
  - Write a clear, concise and short description of what this command does (~10 words); this helps you better
    understanding the command and its purpose.
  ...
  - When executing multiple commands, use `;` to separate them on a single line — it is supported on every host this
    tool runs on. NEVER use newlines to separate commands, newlines are only allowed inside quoted strings. Avoid
    `&&`/`||`: they are unsupported on Windows PowerShell 5.1 (`powershell.exe`, tried before falling back to `pwsh`)
    even though they work on POSIX shells and on PowerShell 7+.
...
## **VERY IMPORTANT**
- This shell session is *NOT* persistent across calls. Each invocation starts a fresh subprocess (`/bin/sh -c` on
...
- Commands specific to one platform (POSIX-only builtins such as `ls`/`grep`, or PowerShell-only cmdlets such as
  `Get-ChildItem`) only work when this tool is running on that platform; check <field>_shell_label</field> above
