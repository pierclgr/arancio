...
1. Path Verification:
   ...
   - The file *MUST* already exist. Use `WriteTool` to create a new file from scratch instead.
2. Read-First Verification:
   - You *MUST* have called `ReadTool` on the same path in this session before invoking `EditTool`.
   - If the file was modified on disk after your last `ReadTool` call (for example by another process or by a `BashCommandTool` invocation), you *MUST* re-read it before editing; the tool rejects edits whose recorded read mtime has drifted.
3. Snippet Construction:
   - Build `old_string` from the content shown by `ReadTool` *after* the `<lineno><TAB>` prefix. *NEVER* include the line-number prefix or tab character in `old_string` — only the actual file content.
...
## USAGE
  ...
  - Prefer `EditTool` over `WriteTool` for incremental modifications to an existing file, since `EditTool` ships only the diff. Use `WriteTool` for full rewrites or new files.
...
- Successful edits update the session's read record automatically with the new file mtime, so an immediately-subsequent `EditTool` or `WriteTool` call on the same path does not require an intervening `ReadTool` call.
