...
2. Read-First Verification:
   ...
   - If the file was modified on disk after your last `ReadFileTool` call (for example by another process or by a `ShellCommandTool` invocation), you *MUST* re-read it before editing; the tool rejects edits whose recorded read mtime has drifted.
...
- *NEVER* attempt to bypass the read-first guard by invoking `ShellCommandTool` (e.g. `sed -i`, `tee`, `awk -i inplace`). Such bypasses defeat the safety mechanism that prevents blind clobbers of files the model has not seen.
