...
2. Read-First Verification (overwriting existing files):
   - If `file_path` points to a file that already exists, you *MUST* have called `ReadTool` on the same path in this session before invoking `WriteTool`.
   - If the file was modified on disk after your last `ReadTool` call (for example by another process or by a `BashCommandTool` invocation), you *MUST* re-read it before overwriting; the tool rejects writes whose recorded read mtime has drifted.
...
## USAGE
  ...
  - Prefer `EditTool` over `WriteTool` for incremental modifications to an existing file, since `EditTool` ships only the diff. Use `WriteTool` for new files or full rewrites.
...
- Successful writes update the session's read record automatically with the new file mtime, so an immediately-subsequent `WriteTool` call on the same path does not require an intervening `ReadTool` call.
