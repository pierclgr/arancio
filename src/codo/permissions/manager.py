...
class PermissionManager:
    ...
    def validate(self, call: ToolCallMessage) -> bool:
        ...
        Returns ``True`` for ``auto`` grants, prompts the user for ``ask``
        grants, and denies calls whose category has no grant.

        Args:
            call: the tool call the agent wants to execute.

        Returns:
            ``True`` when the call may execute, ``False`` otherwise.
        """
